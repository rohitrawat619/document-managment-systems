        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    ...
                    @php
                    $url = $hasPermission
                    ? route('admin.document.notification.create')
                    : 'javascript:void(0);';
                    ...
                    ? "alert('You do not have write permission to perform this action.');"
                    : '';
                    @endphp
                    ...
                    <a href="{{ $url }}"
                        class="{{ $class }}"
                        title="{{ $title }}"
                        @if (!$hasPermission) onclick="{{ $onclick }}" @endif>
                        Add
                    </a>
                </div>
            </div>
                        ...
                        <div class="d-flex  mb-3">
                            <form method="GET" action="{{ route('admin.document.notification.index') }}" class="form-inline" style="width: 100%;">
                                <div class="d-flex mb-3">
                                    ...
                                    <a href="{{ route('admin.document.notification.index') }}" class="btn btn-secondary ms-2">Reset</a>
                                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex order-actions">
                                                    ...
                                                    <a href="{{ route('admin.document.notification.edit', ['id' => base64_encode($r->id)]) }}" title="Edit">
        ...
        $(document).on('click', '.status', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.notification.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
