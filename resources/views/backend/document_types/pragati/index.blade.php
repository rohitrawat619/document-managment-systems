            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Pragati</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-auto">
                <div class="btn-group">
                    <a href="{{route('admin.document.pragati.create')}}" type="button" class="btn btn-primary">Add</a>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->
        <div class="row">
            ...
            <div class="col-xl-12 mx-auto">
                ...
                <div class="card">
                    ...
                    <form method="GET" action="{{ route('admin.document.pragati.index') }}" class="form-inline" style="width: 100%;">
                    <div class="d-flex mb-3">
                    <input type="text" name="search" class="form-control" placeholder="Search by name" value="{{ request()->get('search') }}">
                    <button type="submit" class="btn btn-primary ms-2">Search</button>
                    <a href="{{ route('admin.document.pragati.index') }}" class="btn btn-secondary ms-2">Reset</a>
                    </div>
                        <table class="table mb-0 table-hover table-bordered userTable">
                            ...
                            <tr>
                                ...
                                <td>{{$r->date_of_reply}}</td>
                                <td>{{$r->agenda}}</td>
                                <td>{{$r->issuer_name}}</td>
                                <td>{{$r->issuer_designation}}</td>
                                <td>{{ str_replace(',', ' ', $r->keywords) }}</td>
                                <td>{{date('Y-m-d',strtotime($r->date_of_upload))}}</td>
                                <td>
                                <div class="d-flex order-actions">
                                    <a href="{{route('admin.document.pragati.edit',['id'=>base64_encode($r->id)])}}" class="" title="Edit"><i class="bx bxs-edit"></i></a>
                        ...
                        <div class="d-flex justify-content-center mt-4">
                            {{ $cabinet_note->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
       ...
       $(document).on('click', '.status', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.pragati.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
       ...
       $(document).on('click', '.deleteBtn', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.pragati.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                        },
