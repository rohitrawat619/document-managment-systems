        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    ...
                    @php
                    $url = $hasPermission ? route('admin.document.minutes_of_metting.create') : 'javascript:void(0);';
                    $class = 'btn btn-primary' . ($hasPermission ? '' : ' no-permission');
                    $title = $hasPermission ? 'Add' : 'No Permission';
                    $onclick = $hasPermission ? '' : "onclick=\"alert('You do not have write permission to perform this action.');\"";
                    @endphp

                    <a href="{{ $url }}" class="{{ $class }}" title="{{ $title }}" {!! $onclick !!}>
                        Add
                    </a>
                </div>
            </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex  mb-3">
                            <form method="GET" action="{{ route('admin.document.minutes_of_metting.index') }}" class="form-inline" style="width: 100%;">
                                <div class="d-flex mb-3">
                                    <input type="text" name="search" class="form-control" placeholder="Search by name" value="{{ request()->get('search') }}">
                                    <button type="submit" class="btn btn-primary ms-2">Search</button>
                                    <a href="{{ route('admin.document.minutes_of_metting.index') }}" class="btn btn-secondary ms-2">Reset</a>
                                </div>
                                <table class="table mb-0 table-hover table-bordered userTable">
                                    ...
                                    <tbody>
                                        ...
                                        $userPermissions = session::get('user_permissions');
                                        @endphp
                                        @forelse ($minutes_of_metting as $k => $r)
                                        <tr>
                                            <th scope="row">{{ ($minutes_of_metting->currentPage() - 1) * $minutes_of_metting->perPage() + $k + 1 }}</th>
                                            ...
                                            <td>
                                                <div class="d-flex order-actions">
                                                    ...
                                                    <a href="{{ route('admin.document.minutes_of_metting.edit', ['id' => base64_encode($r->id)]) }}" title="Edit">
                                ...
                                <div class="d-flex justify-content-center mt-4">
                                    {{ $minutes_of_metting->links('pagination::bootstrap-4') }}
                                </div>
                        </div>
                    </div>
                </div>
        ...
        $(document).on('click', '.status', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.minutes_of_metting.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
        ...
        $(document).on('click', '.deleteBtn', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.minutes_of_metting.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                        },
