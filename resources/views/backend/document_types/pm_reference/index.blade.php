            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">PM Reference</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-auto">
                ...
                @endphp
                <a href="{{ in_array(42, $userPermissions) ? route('admin.document.pm_reference.create') : 'javascript:void(0);' }}" 
        ...
        <div class="row">
            ...
            <div class="col-xl-12 mx-auto">
                ...
                <div class="card">
                    ...
                    <form method="GET" action="{{ route('admin.document.pm_reference.index') }}" class="form-inline" style="width: 100%;">
                    <div class="d-flex mb-3">
                    <input type="text" name="search" class="form-control" placeholder="Search by name" value="{{ request()->get('search') }}">
                    <button type="submit" class="btn btn-primary ms-2">Search</button>
                    <a href="{{ route('admin.document.pm_reference.index') }}" class="btn btn-secondary ms-2">Reset</a>
                    </div>
                        <table class="table mb-0 table-hover table-bordered userTable">
                            ...
                            $userPermissions = session::get('user_permissions');
                            @endphp
                            @forelse ($pm_reference as $k => $r)
                            <tr>
                                <!-- Adjust numbering to be continuous across pages -->
                                <th scope="row">{{ ($pm_reference->currentPage() - 1) * $pm_reference->perPage() + $k + 1 }}</th>
                                <td>{{$r->computer_no}}</td>
                                <td>{{$r->file_no}}</td>
                                ...
                                <div class="d-flex order-actions">
                                    <a href="{{route('admin.document.pm_reference.edit',['id'=>base64_encode($r->id)])}}" class="" title="Edit"><i class="bx bxs-edit"></i></a>
                                ...
                                <div class="d-flex order-actions">
                                    @if(in_array(42, $userPermissions))
                                        <a href="{{ route('admin.document.pm_reference.edit', ['id' => base64_encode($r->id)]) }}" title="Edit">
                        ...
                        <div class="d-flex justify-content-center mt-4">
                            {{ $pm_reference->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
       ...
       $(document).on('click', '.status', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.pm_reference.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
       ...
       $(document).on('click', '.deleteBtn', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.pm_reference.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                        },
