...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <div class="btn-group">
                    <a href="{{route('admin.document.guideline.create')}}" type="button" class="btn btn-primary">Add</a>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->
        <div class="row">
            ...
            <div class="col-xl-12 mx-auto">
                ...
                <div class="card">
                    ...
                    <form method="GET" action="{{ route('admin.document.guideline.index') }}" class="form-inline">
                    <div class="d-flex mb-3">
                    ...
                    <button type="submit" class="btn btn-primary ms-2">Search</button>
                        ...
                        <table class="table mb-0 table-hover table-bordered userTable">
                            ...
                            <tbody>
                            @forelse ($guideline as $k => $r)
                            <tr>
                                <!-- Adjust numbering to be continuous across pages -->
                                <th scope="row">{{ ($guideline->currentPage() - 1) * $guideline->perPage() + $k + 1 }}</th>
                                <td>{{$r->computer_no}}</td>
                                <td>{{$r->file_no}}</td>
                                ...
                                <td>
                                    <div class="d-flex order-actions">
                                        <a href="{{route('admin.document.guideline.edit',['id'=>base64_encode($r->id)])}}" class="" title="Edit"><i class="bx bxs-edit"></i></a>
                        ...
                        <div class="d-flex justify-content-center mt-4">
                            {{ $guideline->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
       ...
       $(document).on('click', '.status', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.guideline.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
       ...
       $(document).on('click', '.deleteBtn', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.guideline.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                        },
