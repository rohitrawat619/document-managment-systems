...
$(document).ready(function() {
    ...
    var guidelineStoreUrl = "{{ route('admin.document.guideline.create') }}";
    $('#guidelineForm').on('submit', function(e) {
        e.preventDefault();
        
        let formData = new FormData(this); // FormData object create karein

        $.ajax({
            url: guidelineStoreUrl, // Ensure this route exists
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).ready(function() {
        ...
        $('select[name="user"]').on('change', function() {
            ...
            if(userId) {
                ...
                $.ajax({
                    url: "{{ route('admin.document.guideline.get-divisions-by-user') }}",
                    type: 'GET',
                    data: { user_id: userId },
