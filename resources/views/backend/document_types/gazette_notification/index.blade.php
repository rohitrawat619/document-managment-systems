            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Gazette Notifications</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-auto">
                <div class="btn-group">
                    <a href="{{route('admin.document.gazette_notification.create')}}" type="button" class="btn btn-primary">Add</a>
                </div>
            </div>
        </div>
        <!--end breadcrumb-->
        <div class="row">
            ...
            <div class="col-xl-12 mx-auto">
                ...
                <div class="card">
                    ...
                    <form method="GET" action="{{ route('admin.document.gazette_notification.index') }}" class="form-inline" style="width: 100%;">
                    <div class="d-flex mb-3">
                    <input type="text" name="search" class="form-control" placeholder="Search by name" value="{{ request()->get('search') }}">
                    <button type="submit" class="btn btn-primary ms-2">Search</button>
                    <a href="{{ route('admin.document.gazette_notification.index') }}" class="btn btn-secondary ms-2">Reset</a>
                     </div>
                        <table class="table mb-0 table-hover table-bordered userTable">
                            ...
                            <tbody>
                            @forelse ($gazette_notification as $k => $r)
                                <tr>
                                    <th scope="row">{{ ($gazette_notification->currentPage() - 1) * $gazette_notification->perPage() + $k + 1 }}</th>
                                    <td>{{ $r->notification_no }}</td>
                                    <td>{{ $r->title }}</td>
                                    <td>{{ date('Y-m-d', strtotime($r->date_of_notification)) }}</td>
                                    <td>{{ $r->issuer_name }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex order-actions">
                                            <a href="{{ route('admin.document.gazette_notification.edit', ['id' => base64_encode($r->id)]) }}" class="" title="Edit"><i class="bx bxs-edit"></i></a>
                         ...
                         <div class="d-flex justify-content-center mt-4">
                            {{ $gazette_notification->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
       ...
       $(document).on('click', '.status', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.document.gazette_notification.status')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                            'type': type
       ...
       $(document).on('click', '.deleteBtn', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: "{{route('admin.gazette_notification.delete')}}",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
                        },
